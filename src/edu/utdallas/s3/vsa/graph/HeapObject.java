...
import edu.utdallas.s3.vsa.graph.specialcase.FindInClassAdd;
...
public class HeapObject implements IDGNode {
	...
	/*
	 * @Override public HashSet<DataSourceType> getDataSrcs() { Set<IDGNode> dps
	 * = new HashSet<IDGNode>(); getDirectAndIndirectDependents(dps);
	 * 
	 * HashSet<DataSourceType> toRet = new HashSet<DataSourceType>(); for
	 * (IDGNode i : dps) { if (i instanceof ValuePoint) { for (BackwardContext
	 * bc : ((ValuePoint) i).getBcs()) { toRet.addAll(bc.getDataSrcs()); } } }
	 * return toRet; }
	 */
}
