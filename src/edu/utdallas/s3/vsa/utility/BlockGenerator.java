...
public class BlockGenerator {
	...
	public static List<Block> removeBlocksThatHaveBeenVisitedTwice(ArrayList<Pair<Block, Integer>> bks, List<Block> targets) {
		...
		// get blocks in this method
		// a[1] b[2] c[2] a[1] d[1] e[0] f[1] g[1] e[0]
		...
		/*
		for (Pair<Block, Integer> tb : bks) {
			Logger.printW("  " + tb.getValue0().getIndexInMethod() + " " + tb.getValue1() + " " + tb.getValue0().getBody().getMethod().toString());
		}
		Logger.printW("res:");
		
		for (Block tb : toRet) {
			Logger.printW("  " + tb.getIndexInMethod() + " " + tb.getBody().getMethod().toString());
		}
		*/
		return toRet;
	}
	/*
	 * for (Pair<Block, Integer> tb : bks) {
	 * Logger.printW("  "+tb.getValue0().getIndexInMethod
	 * ()+" "+tb.getValue1()+" "
	 * +tb.getValue0().getBody().getMethod().toString()); }
	 * Logger.printW(">>>>>>>>"); for (Pair<Block, Integer> tb : hit) {
	 * Logger.printW
	 * ("  "+tb.getValue0().getIndexInMethod()+" "+tb.getValue1()+" "
	 * +tb.getValue0().getBody().getMethod().toString()); }
	 */
}
